...
export default function EditarProdutoPage({ params }) {
  ...
  useEffect(() => {
    adminService.produtos
      .listar()
      .then((lista) => {
        const produto = Array.isArray(lista) ? lista.find((p) => String(p.id) === String(id)) : null;
        if (produto) {
          setForm({
            nome: produto.nome ?? "",
          ...
          if (produto.imagemBase64) setImagemAtual(`data:image/*;base64,${produto.imagemBase64}`);
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    setSalvando(true);
    try {
      await adminService.produtos.atualizar(
        id,
        {
          nome: form.nome,
  ...
  const imagemExibida = preview ?? imagemAtual;
  ...
  return (
    <div>
      <div style={{ marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <h2 style={{ fontSize: 18, fontWeight: 600 }}>Editar Produto</h2>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <label style={{
              ...
              background: imagemExibida ? "#f9f9f9" : "#fafafa", gap: 8,
            }}>
              {imagemExibida ? (
                <img src={imagemExibida} alt="preview" style={{ maxHeight: 160, maxWidth: "100%", borderRadius: 6, objectFit: "contain" }} />
              ) : (
                <>
                  <span style={{ fontSize: 32 }}>📷</span>
                  <span style={{ fontSize: 13, color: "#888" }}>Clique para trocar a imagem</span>
            ...
            {imagemExibida && (
              <button type="button" onClick={() => { setImagem(null); setPreview(null); setImagemAtual(null); }}
